...
class RegForm(forms.Form):
    username = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': '请输入用户名(3~30位)'}
            )
        )
    ...
    def clean(self):
        code = self.request.session.get('register_code', '')    # 这是服务器生成的码
        verification_code = self.cleaned_data.get('verification_code', '')   # 这是你收到邮件后填的码
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        # 判断验证码
        code = self.request.session.get('bind_email_code', '')    # 这是服务器生成的码
        verification_code = self.cleaned_data.get('verification_code', '')   # 这是你收到邮件后填的码
        if not(code != '' and code == verification_code):
            raise forms.ValidationError('验证码不正确')
        return self.cleaned_data
...
class ChangePasswordForm(forms.Form):
    old_password = forms.CharField(
        ...
        min_length = 6,
        widget=forms.PasswordInput(
            attrs={'class': 'form-control', 'placeholder': '请输入原来的密码'}
            )
        )
    ...
    new_password_again = forms.CharField(
        ...
        min_length = 6,
        widget=forms.PasswordInput(
            attrs={'class': 'form-control', 'placeholder': '再次确认新的密码'}
            )
        )  
